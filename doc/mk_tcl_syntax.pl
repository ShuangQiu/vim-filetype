  ...
  while (<PROC>) {
    ...
    if (/^\s*&?define_proc_attributes\s+(\S+)/) {
      ...
      if (/-define_args/) {
        ...
        # Add -noXXX stuff for booleans
        foreach $item (m/\{\s*(-[^\}]+)/g) {
          if ($item =~ /\s+boolean\s+(required|optional)/) {
            print STDERR "item is a boolean \"$item\"\n" if ($debug);
            ($temp = $item) =~ s/\s*-(\S+).*/\1/;
            $items = "$items -no${temp}";
...
sub show_help {
 print "Syntax:
  ...
  -verbose	Be chatty
  -procfile file Use procs file \"file\" (can be invoked multiple times).
